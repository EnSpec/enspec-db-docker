...
CREATE TABLE radiometric_calibration_target (
  ...
  radiometric_target_center GEOMETRY(POINT,4326) NOT NULL,
  radiometric_target_poly GEOMETRY(POLYGON,4326) NOT NULL,
...
CREATE OR REPLACE VIEW radiometric_calibration_target_view AS
  SELECT
    ...
    ST_X(r.radiometric_target_center)  as rt_center_long,
    ST_Y(r.radiometric_target_center)  as rt_center_lat,
...
BEGIN
  SELECT ST_SetSRID(ST_MakePoint(rt_center_long, rt_center_lat), 4326) INTO radiometric_target_center_geom;
...
radiometric_target_poly_geom GEOMETRY;
BEGIN
  SELECT ST_SetSRID(ST_MakePoint(rt_center_long_in, rt_center_lat_in),4326) INTO radiometric_target_center_geom;
...
BEGIN
  ...
  IF (rid IS NULL) THEN
    RAISE EXCEPTION 'Unknown radiometric_calibration_target: calibration_date="%" collection_start_time="%" collection_stop_time="%" rt_center_long="%" rt_center_lat="%" radiometric_target_poly_kml="%"
    raw_file_loc="%" processed_spectra_loc="%"', calibration_date_in, collection_start_time_in, collection_stop_time_in, rt_center_long_in, rt_center_lat_in, radiometric_target_poly_kml_in, raw_file_loc_in, processed_spectra_loc_in;
  END IF;
