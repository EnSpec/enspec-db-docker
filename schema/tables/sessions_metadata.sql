...
BEGIN
  ...
  SELECT get_sessions_id(session_name, start_time, end_time, line_count, bad_lines) INTO sess_id;
...
BEGIN
  SELECT get_sessions_id(session_name_in, start_time_in, end_time_in, line_count_in, bad_lines_in) INTO sess_id;
...
CREATE OR REPLACE FUNCTION get_sessions_metadata_id(
  session_name_in text, start_time_in time, end_time_in time, line_count_in float, bad_lines_in text,
...
BEGIN
  ...
  IF (sid IS NULL) THEN
    RAISE EXCEPTION 'Unknown sessions_metadata: session_name="%" start_time="%" end_time="%" line_count="%" bad_lines="%"
    variable_name="%" variable_type="%" units_name="%" units_type="%" units_description="%" value="%"', session_name_in,
    start_time_in, end_time_in, line_count_in, bad_lines_in, variable_name_in, variable_type_in, units_name_in,
