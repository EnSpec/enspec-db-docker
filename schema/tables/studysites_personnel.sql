-- TABLE
DROP TABLE IF EXISTS studysites_personnel CASCADE;
CREATE TABLE studysites_personnel (
  studysites_personnel_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  source_id UUID REFERENCES source NOT NULL,
  study_sites_id UUID REFERENCES study_sites NOT NULL,
  personnel_id UUID REFERENCES personnel NOT NULL,
  activity_id UUID REFERENCES activity NOT NULL,
  activity_date_started DATE NOT NULL,
  activity_date_completed DATE,
  activity_lead TEXT NOT NULL
);
CREATE INDEX studysites_personnel_source_id_idx ON studysites_personnel(source_id);
CREATE INDEX studysites_personnel_studysites_id_idx ON studysites_personnel(study_sites_id);
CREATE INDEX studysites_personnel_personnel_id_idx ON studysites_personnel(personnel_id);
CREATE INDEX studysites_personnel_activity_id_idx ON studysites_personnel(activity_id);

-- VIEW
CREATE OR REPLACE VIEW studysites_personnel_view AS
  SELECT
    s.studysites_personnel_id AS studysites_personnel_id,
    ss.site_name  as site_name,
    ss.region  as region,
    ST_AsKML(ss.site_poly)  as site_poly_kml,
    per.personnel_name  as personnel_name,
    per.personnel_role  as personnel_role,
    per.organization  as organization,
    per.office_phone  as office_phone,
    per.cell_phone  as cell_phone,
    per.email  as email,
    per.personnel_notes  as personnel_notes,
    ac.activity  as activity,
    ac.activity_description  as activity_description,
    s.activity_date_started  as activity_date_started,
    s.activity_date_completed  as activity_date_completed,
    s.activity_lead  as activity_lead,

    sc.name AS source_name
  FROM
    studysites_personnel s
LEFT JOIN source sc ON s.source_id = sc.source_id
LEFT JOIN study_sites ss ON s.study_sites_id = ss.study_sites_id
LEFT JOIN personnel per ON s.personnel_id = per.personnel_id
LEFT JOIN activity ac ON s.activity_id = ac.activity_id;

-- FUNCTIONS
CREATE OR REPLACE FUNCTION insert_studysites_personnel (
  studysites_personnel_id UUID,
  site_name TEXT,
  region TEXT,
  site_poly_kml TEXT,
  personnel_name TEXT,
  personnel_role TEXT,
  organization TEXT,
  office_phone TEXT,
  cell_phone TEXT,
  email TEXT,
  personnel_notes TEXT,
  activity TEXT,
  activity_description TEXT,
  activity_date_started DATE,
  activity_date_completed DATE,
  activity_lead TEXT,

  source_name TEXT) RETURNS void AS $$
DECLARE
  source_id UUID;
  ssid UUID;
  perid UUID;
  acid UUID;
BEGIN
  SELECT get_study_sites_id(site_name, region, site_poly_kml) INTO ssid;
  SELECT get_personnel_id(personnel_name, personnel_role, organization, office_phone, cell_phone, email, personnel_notes) INTO perid;
  SELECT get_activity_id(activity, activity_description) INTO acid;
  IF( studysites_personnel_id IS NULL ) THEN
    SELECT uuid_generate_v4() INTO studysites_personnel_id;
  END IF;
  SELECT get_source_id(source_name) INTO source_id;

  INSERT INTO studysites_personnel (
    studysites_personnel_id, study_sites_id, personnel_id, activity_id, activity_date_started, activity_date_completed, activity_lead, source_id
  ) VALUES (
    studysites_personnel_id, ssid, perid, acid, activity_date_started, activity_date_completed, activity_lead, source_id
  );

EXCEPTION WHEN raise_exception THEN
  RAISE;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION update_studysites_personnel (
  studysites_personnel_id_in UUID,
  site_name_in TEXT,
  region_in TEXT,
  site_poly_kml_in TEXT,
  personnel_name_in TEXT,
  personnel_role_in TEXT,
  organization_in TEXT,
  office_phone_in TEXT,
  cell_phone_in TEXT,
  email_in TEXT,
  personnel_notes_in TEXT,
  activity_in TEXT,
  activity_description_in TEXT,
  activity_date_started_in DATE,
  activity_date_completed_in DATE,
  activity_lead_in TEXT) RETURNS void AS $$
DECLARE
ssid UUID;
perid UUID;
acid UUID;
BEGIN
  SELECT get_study_sites_id(site_name_in, region_in, site_poly_kml_in) INTO ssid;
  SELECT get_personnel_id(personnel_name_in, personnel_role_in, organization_in, office_phone_in, cell_phone_in, email_in, personnel_notes_in) INTO perid;
  SELECT get_activity_id(activity_in, activity_description_in) INTO acid;
  UPDATE studysites_personnel SET (
    study_sites_id, personnel_id, activity_id, activity_date_started, activity_date_completed, activity_lead
  ) = (
    study_sites_id_in, perid, acid, activity_date_started_in, activity_date_completed_in, activity_lead_in
  ) WHERE
    studysites_personnel_id = studysites_personnel_id_in;

EXCEPTION WHEN raise_exception THEN
  RAISE;
END;
$$ LANGUAGE plpgsql;

-- FUNCTION TRIGGERS
CREATE OR REPLACE FUNCTION insert_studysites_personnel_from_trig()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM insert_studysites_personnel(
    studysites_personnel_id := NEW.studysites_personnel_id,
    site_name := NEW.site_name,
    region := NEW.region,
    site_poly_kml := NEW.site_poly_kml,
    personnel_name := NEW.personnel_name,
    personnel_role := NEW.personnel_role,
    organization := NEW.organization,
    office_phone := NEW.office_phone,
    cell_phone := NEW.cell_phone,
    email := NEW.email,
    personnel_notes := NEW.personnel_notes,
    activity := NEW.activity,
    activity_description := NEW.activity_description,
    activity_date_started := NEW.activity_date_started,
    activity_date_completed := NEW.activity_date_completed,
    activity_lead := NEW.activity_lead,

    source_name := NEW.source_name
  );
  RETURN NEW;

EXCEPTION WHEN raise_exception THEN
  RAISE;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION update_studysites_personnel_from_trig()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM update_studysites_personnel(
    studysites_personnel_id_in := NEW.studysites_personnel_id,
    site_name_in := NEW.site_name,
    region_in := NEW.region,
    site_poly_kml_in := NEW.site_poly_kml,
    personnel_name_in := NEW.personnel_name,
    personnel_role_in := NEW.personnel_role,
    organization_in := NEW.organization,
    office_phone_in := NEW.office_phone,
    cell_phone_in := NEW.cell_phone,
    email_in := NEW.email,
    personnel_notes_in := NEW.personnel_notes,
    activity_in := NEW.activity,
    activity_description_in := NEW.activity_description,
    activity_date_started_in := NEW.activity_date_started,
    activity_date_completed_in := NEW.activity_date_completed,
    activity_lead_in := NEW.activity_lead
  );
  RETURN NEW;

EXCEPTION WHEN raise_exception THEN
  RAISE;
END;
$$ LANGUAGE plpgsql;

-- FUNCTION GETTER
CREATE OR REPLACE FUNCTION get_studysites_personnel_id(
  site_name_in text, region_in text, site_poly_kml_in text,
  personnel_name_in TEXT,
  personnel_role_in TEXT,
  organization_in TEXT,
  office_phone_in TEXT,
  cell_phone_in TEXT,
  email_in TEXT,
  personnel_notes_in TEXT,
  activity_in TEXT,
  activity_description_in TEXT,
  activity_date_started_in DATE,
  activity_date_completed_in DATE,
  activity_lead_in TEXT
) RETURNS UUID AS $$
DECLARE
  sid UUID;
  ssid UUID;
  perid UUID;
  acid UUID;
BEGIN
  SELECT get_study_sites_id(site_name_in, region_in, site_poly_kml_in) INTO ssid;
  SELECT get_personnel_id(personnel_name_in, personnel_role_in, organization_in, office_phone_in, cell_phone_in, email_in, personnel_notes_in) INTO perid;
  SELECT get_activity_id(activity_in, activity_description_in) INTO acid;


  IF (activity_date_completed_in IS NULL) THEN
    SELECT
      studysites_personnel_id INTO sid
    FROM
      studysites_personnel s
    WHERE
      study_sites_id = ssid AND
      personnel_id = perid AND
      activity_id = acid AND
      activity_date_started = activity_date_started_in AND
      activity_date_completed is NULL AND
      activity_lead = activity_lead_in;
  ELSE
    SELECT
      studysites_personnel_id INTO sid
    FROM
      studysites_personnel s
    WHERE
      study_sites_id = ssid AND
      personnel_id = perid AND
      activity_id = acid AND
      activity_date_started = activity_date_started_in AND
      activity_date_completed = activity_date_completed_in AND
      activity_lead = activity_lead_in;

  END IF;

  IF (sid IS NULL) THEN
    RAISE EXCEPTION 'Unknown studysites_personnel: site_name="%" region="%" site_poly_kml="%" personnel_name="%" personnel_role="%"
    organization="%" office_phone="%" cell_phone="%" email="%" personnel_notes="%"
    activity="%" activity_description="%" activity_date_started ="%" activity_date_completed="%" activity_lead="%"',
    site_name_in, region_in, site_poly_kml_in, personnel_name_in, personnel_role_in, organization_in,
    office_phone_in, cell_phone_in, email_in, personnel_notes_in, activity_in, activity_description_in, activity_date_started_in,
    activity_date_completed_in, activity_lead_in;
  END IF;

  RETURN sid;
END ;
$$ LANGUAGE plpgsql;

-- RULES
CREATE TRIGGER studysites_personnel_insert_trig
  INSTEAD OF INSERT ON
  studysites_personnel_view FOR EACH ROW
  EXECUTE PROCEDURE insert_studysites_personnel_from_trig();

CREATE TRIGGER studysites_personnel_update_trig
  INSTEAD OF UPDATE ON
  studysites_personnel_view FOR EACH ROW
  EXECUTE PROCEDURE update_studysites_personnel_from_trig();
